...
async (oshiya, mek, m, { from, body, isOwner }) => {
    ...
    for (const text in data) {
        if (body.toLowerCase() === text.toLowerCase()) {
            ...
            if (config.AUTO_VOICE === 'true') {
                ...
                await oshiya.sendMessage(from, { audio: { url: data[text] }, mimetype: 'audio/mpeg', ptt: true }, { quoted: mek });
            }
        }
    }                
});
    ...
    for (const text in data) {
        if (body.toLowerCase() === text.toLowerCase()) {
            ...
            if (config.AUTO_STICKER === 'true') {
                //if (isOwner) return;        
                await oshiya.sendMessage(from,{sticker: { url : data[text]},package: 'S_I_H_I_L_E_L'},{ quoted: mek })   
